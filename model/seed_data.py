...
Covers:
  - Australian GP: 2022, 2023, 2024, 2026  (track-history + current-season signal)
...
def _d(driver_id, full_name, team, q_pos, r_pos, grid=None, dnf=False):
    return {
        ...
        "quali_q2_time": None,
        "quali_q3_time": None,
        "quali_no_time": q_pos is None,
        "quali_dsq": False,
        "quali_reached_q2": q_pos is not None and q_pos <= 15,
        "quali_reached_q3": q_pos is not None and q_pos <= 10,
        "sprint_pos": None,
        "sprint_grid_pos": None,
        "sprint_dnf": False,
        "sprint_dsq": False,
        "sprint_fastest_lap": False,
        "sprint_overtakes": 0,
        "race_pos"   : r_pos if not dnf else None,
        "grid_pos"   : grid or q_pos,
        "race_fastest_lap": False,
        "race_overtakes": 0,
        "driver_of_day": False,
        "race_dsq": False,
        "dnf"        : dnf,
...
SEEDED_WEEKENDS = [
    ...
    # =========================================================
    # 2026 Australian GP (Round 1) — Albert Park
    # Russell leads Antonelli for Mercedes 1-2; Ferrari 3-4
    # Fantasy scores are loaded from the official game totals after Australia.
    # =========================================================
    {
        "season": 2026, "round": 1,
        "race_name": "Australian Grand Prix",
        "circuit"  : "Albert Park Grand Prix Circuit",
        "drivers"  : [
            _d("george_russell",    "George Russell",    "Mercedes",         1,  1, grid=1),
            _d("kimi_antonelli",    "Kimi Antonelli",    "Mercedes",         2,  2, grid=2),
            _d("charles_leclerc",   "Charles Leclerc",   "Ferrari",          4,  3, grid=4),
            _d("lewis_hamilton",    "Lewis Hamilton",    "Ferrari",          7,  4, grid=7),
            _d("lando_norris",      "Lando Norris",      "McLaren",          6,  5, grid=6),
            _d("max_verstappen",    "Max Verstappen",    "Red Bull Racing", 20,  6, grid=20),
            _d("oliver_bearman",    "Oliver Bearman",    "Haas F1 Team",    12,  7, grid=12),
            _d("arvid_lindblad",    "Arvid Lindblad",    "Racing Bulls",     9,  8, grid=9),
            _d("gabriel_bortoleto", "Gabriel Bortoleto", "Audi",            10,  9, grid=10),
            _d("pierre_gasly",      "Pierre Gasly",      "Alpine",          14, 10, grid=14),
            _d("esteban_ocon",      "Esteban Ocon",      "Haas F1 Team",    13, 11, grid=13),
            _d("alexander_albon",   "Alexander Albon",   "Williams",        15, 12, grid=15),
            _d("liam_lawson",       "Liam Lawson",       "Racing Bulls",     8, 13, grid=8),
            _d("franco_colapinto",  "Franco Colapinto",  "Alpine",          16, 14, grid=16),
            _d("carlos_sainz",      "Carlos Sainz",      "Williams",        21, 15, grid=21),
            _d("sergio_perez",      "Sergio Perez",      "Cadillac",        18, 16, grid=18),
            _d("lance_stroll",      "Lance Stroll",      "Aston Martin",    22, 17, grid=22),
            _d("fernando_alonso",   "Fernando Alonso",   "Aston Martin",    17, None, grid=17, dnf=True),
            _d("valtteri_bottas",   "Valtteri Bottas",   "Cadillac",        19, None, grid=19, dnf=True),
            _d("isack_hadjar",      "Isack Hadjar",      "Red Bull Racing",  3, None, grid=3, dnf=True),
            _d("oscar_piastri",     "Oscar Piastri",     "McLaren",          5, None, grid=5, dnf=True),
            _d("nico_hulkenberg",   "Nico Hulkenberg",   "Audi",            11, None, grid=11, dnf=True),
        ],
        "scores": {
            "Max Verstappen": 50.0,
            "George Russell": 39.0,
            ...
            "Valtteri Bottas": -16.0,
            "Nico Hulkenberg": -20.0,
            "Lance Stroll": -23.0,
        },
        "constructor_scores": {
            "Mercedes": 96.0,
            "Ferrari": 69.0,
            "Red Bull Racing": 42.0,
            "Racing Bulls": 35.0,
            "Haas F1 Team": 34.0,
            "Alpine": 22.0,
            "Williams": 20.0,
    ...
    # =========================================================
    # 2026 Chinese GP (Round 2) — Shanghai International Circuit
    # Sprint race weekend. Antonelli pole + race win + fastest lap.
    # Russell wins sprint. McLaren/Albon DNS in race after sprint.
    # =========================================================
    {
        "season": 2026, "round": 2,
        ...
        "drivers"  : [
            _d("kimi_antonelli",    "Kimi Antonelli",    "Mercedes",          1,  1, grid=1),
            _d("george_russell",    "George Russell",    "Mercedes",          2,  2, grid=2),
            _d("lewis_hamilton",    "Lewis Hamilton",    "Ferrari",           3,  3, grid=3),
            _d("charles_leclerc",   "Charles Leclerc",   "Ferrari",           4,  4, grid=4),
            _d("oliver_bearman",    "Oliver Bearman",    "Haas F1 Team",     10,  5, grid=10),
            _d("pierre_gasly",      "Pierre Gasly",      "Alpine",            7,  6, grid=7),
            _d("liam_lawson",       "Liam Lawson",       "Racing Bulls",     14,  7, grid=14),
            _d("isack_hadjar",      "Isack Hadjar",      "Red Bull Racing",   9,  8, grid=9),
            _d("carlos_sainz",      "Carlos Sainz",      "Williams",         17,  9, grid=17),
            _d("franco_colapinto",  "Franco Colapinto",  "Alpine",           12, 10, grid=12),
            _d("nico_hulkenberg",   "Nico Hulkenberg",   "Audi",             11, 11, grid=11),
            _d("arvid_lindblad",    "Arvid Lindblad",    "Racing Bulls",     15, 12, grid=15),
            _d("valtteri_bottas",   "Valtteri Bottas",   "Cadillac",         20, 13, grid=20),
            _d("esteban_ocon",      "Esteban Ocon",      "Haas F1 Team",     13, 14, grid=13),
            _d("sergio_perez",      "Sergio Perez",      "Cadillac",         22, 15, grid=22),
            _d("max_verstappen",    "Max Verstappen",    "Red Bull Racing",   8, None, grid=8,  dnf=True),
            _d("fernando_alonso",   "Fernando Alonso",   "Aston Martin",     19, None, grid=19, dnf=True),
            _d("lance_stroll",      "Lance Stroll",      "Aston Martin",     21, None, grid=21, dnf=True),
            _d("oscar_piastri",     "Oscar Piastri",     "McLaren",           5, None, grid=5,  dnf=True),
            _d("lando_norris",      "Lando Norris",      "McLaren",           6, None, grid=6,  dnf=True),
            _d("gabriel_bortoleto", "Gabriel Bortoleto", "Audi",             16, None, grid=16, dnf=True),
            _d("alexander_albon",   "Alexander Albon",   "Williams",         18, None, grid=18, dnf=True),
        ],
        "scores": {
            "Kimi Antonelli"   : 68.0,
            "George Russell"   : 45.0,
            ...
            "Pierre Gasly"     : 20.0,
            "Sergio Perez"     : 20.0,
            "Franco Colapinto" : 18.0,
            "Arvid Lindblad"   :  7.0,
            "Isack Hadjar"     : 19.0,
            "Lando Norris"     : -10.0,
            "Alexander Albon"  :  -7.0,
            "Gabriel Bortoleto": -14.0,
            "Valtteri Bottas"  :   3.0,
            "Nico Hulkenberg"  :   7.0,
            "Oscar Piastri"    :  -7.0,
            "Fernando Alonso"  :  -7.0,
